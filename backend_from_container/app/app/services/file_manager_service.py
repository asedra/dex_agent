...
class FileManagerService:
    ...
    async def download_file(
        ...
        try:
            ...
            # Handle case where we have mock data
            if download_id == "mock_download_123":
                # Generate mock file content
                mock_content = b"Mock file content for testing\nThis is a test file.\n"
                yield mock_content
                return
            
            # Receive file chunks from real agent
            for chunk_num in range(total_chunks):
                try:
                    ...
                    if chunk_response.get("data"):
                        ...
                        # If no data in chunk response, something went wrong
                        raise HTTPException(
                            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                            detail=f"Failed to receive chunk {chunk_num}"
                        )
                except Exception as e:
                    raise HTTPException(
                        status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                        detail=f"Error downloading chunk {chunk_num}: {str(e)}"
                    )
    ...
    def _generate_mock_response(self, command: Dict[str, Any]) -> Dict[str, Any]:
        ...
        elif op_type == "file_chunk_request":
            # Return mock chunk data
            mock_data = "Mock file content for testing\nThis is a test file.\n"
            return {
                "success": True,
                "data": mock_data.encode().hex()
            }
        else:
            return {
                "success": True,
