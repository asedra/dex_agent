...
class PowerShellService:
    ...
    @staticmethod
    async def execute_command_on_agent(agent_id: str, command: str, db) -> dict:
        """
        Execute a PowerShell command on a specific agent via WebSocket
        """
        try:
            logger.info(f"Executing command on agent {agent_id}: {command[:100]}...")
            
            # Use the websocket manager to execute command on agent
            result = await websocket_manager.execute_command_on_agent(
                agent_id=agent_id,
                command=command,
                command_type="powershell"
            )
            
            # Convert WebSocket manager response to expected format
            return {
