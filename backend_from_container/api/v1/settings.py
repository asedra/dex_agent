...
async def test_chatgpt_api(token: str = Depends(verify_token)):
    ...
    try:
        # Get API key - first check environment, then database
        ...
        if not api_key:
            ...
            if not api_key_setting:
                raise HTTPException(status_code=400, detail="ChatGPT API key not configured in environment variable or database settings")
