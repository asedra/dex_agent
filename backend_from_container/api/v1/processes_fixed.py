...
async def get_process_tree(
    ...
    try:
        ...
        # Using a more compatible approach with WMI
        ps_command = '''
        try {
            ...
            foreach ($proc in $processes) {
                $processTree[[string]$proc.ProcessId] = @{
                    ...
                    'CreationDate' = if ($proc.CreationDate) { $proc.CreationDate.ToString() } else { $null }
            ...
            try {
                ...
                foreach ($proc in $processes) {
                    $simpleTree[[string]$proc.Id] = @{
                        ...
                        'CreationDate' = if ($proc.StartTime) { $proc.StartTime.ToString() } else { $null }
                        'WorkingSetSize' = $proc.WorkingSet
                        'Children' = @()
                    }
                }
        ...
        if response and response.get("success"):
            ...
            try:
                ...
                if isinstance(tree_data, dict) and tree_data.get("error"):
                    ...
                    raise HTTPException(
                        status_code=500, 
                        detail=f"Failed to get process tree: {tree_data.get('message', 'Unknown error')}"
                    )
                ...
                logger.error(f"Failed to parse process tree data: {str(e)}, output: {output[:500]}")
