...
async def get_agent_services(
    ...
    try:
        ...
        # Execute PowerShell command to get services - with pagination at PowerShell level
        ps_command = """
        Get-Service"""
        if filter:
            ps_command += f" | Where-Object {{$_.Name -like '*{filter}*' -or $_.DisplayName -like '*{filter}*'}}"
        ps_command += f""" | Select-Object -Skip {(page - 1) * page_size} -First {page_size} | ForEach-Object {{
            [PSCustomObject]@{{
                name = $_.Name
        ...
        if response and response.get("success"):
            try:
                ...
                # Since we're paginating at PowerShell level, we estimate total count
                # This is approximate - we'd need a separate count query for exact number
                total_services = len(services_data) + ((page - 1) * page_size)
                # If we got a full page, assume there might be more
                if len(services_data) == page_size:
                    total_services = total_services + page_size  # Estimate
                    
                total_pages = max(1, page + (1 if len(services_data) == page_size else 0))
                ...
                return {
                    ...
                    "has_next": len(services_data) == page_size,
                    "has_prev": page > 1,
                    "agent_id": agent_id,
                    "timestamp": datetime.now().isoformat()
                }
            except json.JSONDecodeError as e:
                output_data = response.get("output", "") if "output" in response else str(response.get("data", ""))
                logger.error(f"Failed to parse service data: {str(e)}, output: {output_data}")
