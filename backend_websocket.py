...
# Store dashboard connections separately
...
async def handle_agent_message(agent_id: str, message: Dict[str, Any]):
    ...
    if message_type == "heartbeat":
        ...
        if command_id:
            ...
            # DX-227 fix: Convert Python dict format (single quotes) to JSON format (double quotes)
            if output_value and isinstance(output_value, str):
                # Check if output looks like a Python dict with single quotes
                if output_value.strip().startswith("{") and "'" in output_value[:50]:
                    try:
                        # Try to safely evaluate as Python literal and convert to JSON
                        import ast
                        python_obj = ast.literal_eval(output_value)
                        output_value = json.dumps(python_obj)
                        logger.info("Converted Python dict format to JSON format for command output")
                    except (ValueError, SyntaxError) as e:
                        logger.warning(f"Could not convert Python dict to JSON: {e}")
        ...
        # Initialize variables outside the if block to avoid undefined variable errors
        error_msg = ""
        execution_time = 0.0
        output_data = ""
    ...
    elif message_type == "software_inventory":
        # Handle software inventory update from agent
        inventory_data = message.get("data", {})
        software_list = inventory_data.get("software_list", [])
        
        logger.info(f"Software inventory received from agent {agent_id}: {len(software_list)} items")
        
        try:
            # Import software service
            from ...services.software_service import SoftwareService
            software_service = SoftwareService()
            
            # Sync software inventory for the agent
            result = await software_service.sync_agent_inventory(agent_id, software_list)
            logger.info(f"Software inventory synced for agent {agent_id}: {result}")
            
        except Exception as e:
            logger.error(f"Error syncing software inventory for agent {agent_id}: {e}")
...
async def client_websocket_endpoint(websocket: WebSocket, token: str = None):
    """WebSocket endpoint for frontend client communication"""
    connection_id = None
    ...
    try:
        ...
        # Register client connection (not an agent)
        connection_id = await websocket_manager.connect(websocket, agent_id=None, accept=False)
        ...
        while True:
            try:
                # Receive message from client (ping, etc.)
                data = await websocket.receive_text()
                message = json.loads(data)
        ...
        if connection_id:
            websocket_manager.disconnect(connection_id)
            logger.info("Client WebSocket connection cleaned up")
...
async def dashboard_websocket_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        logger.info(f"Dashboard WebSocket connection established: {connection_id}")
        ...
        while True:
            try:
                ...
                if message_type == "subscribe":
                    ...
                    logger.info(f"Dashboard subscribed to channel: {channel}")
                    ...
                    channel = message.get("data", {}).get("channel")
                    logger.info(f"Dashboard unsubscribed from channel: {channel}")
        ...
        # Clean up connection
        if connection_id and connection_id in dashboard_connections:
            del dashboard_connections[connection_id]
            logger.info(f"Dashboard connection {connection_id} cleaned up")

async def broadcast_to_dashboards(message: Dict[str, Any]):
    ...
    for conn_id, websocket in dashboard_connections.items():
        try:
            await websocket.send_text(message_str)
        except Exception as e:
            logger.error(f"Failed to send to dashboard {conn_id}: {str(e)}")
            disconnected.append(conn_id)
    
    # Clean up disconnected connections
    for conn_id in disconnected:
        del dashboard_connections[conn_id]

# Helper function to send dashboard updates
async def send_dashboard_update(update_type: str, data: Any):
