#!/usr/bin/env python3
"""
Modern DexAgents Windows Agent GUI (Simplified Version)
A modern, robust agent installation and management interface without psutil dependency
"""
...
class SimpleSystemMonitor:
    ...
    @staticmethod
    def get_cpu_usage():
        """Get CPU usage using Windows Management Instrumentation"""
        try:
            import subprocess
    ...
    def get_memory_usage():
        """Get memory usage using Windows Management Instrumentation"""
        try:
            import subprocess
            result = subprocess.run(
...
class ModernDexAgentGUI:
    ...
    def __init__(self):
        ...
        self.logger.info("Starting Modern DexAgents Windows Agent GUI (Simplified)")
    ...
    def create_title_section(self, parent):
        ...
        # Version info
        version_label = ttk.Label(title_frame, text=f"Version {self.config['version']} (Simplified)", style='Status.TLabel')
    ...
    def create_log_section(self, parent):
        ...
        self.log_message("Modern DexAgents Windows Agent GUI started (Simplified)")
    ...
    def get_system_info(self) -> Optional[Dict[str, Any]]:
        """Get comprehensive system information without psutil"""
        try:
            # Basic system info
            ...
            # Performance metrics using simple monitor
            cpu_usage = SimpleSystemMonitor.get_cpu_usage()
            memory_usage = SimpleSystemMonitor.get_memory_usage()
            disk_usage = SimpleSystemMonitor.get_disk_usage()
        ...
        def update_status_display():
            try:
                ...
                if system_info:
                    ...
                    self.disk_usage_label.config(text=f"{system_info['disk_usage']:.1f}%")
